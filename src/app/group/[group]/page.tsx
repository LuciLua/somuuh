export default function group({ params }) {
    return (
        <div className="bg-zinc-800 h-fit relative">
            <header className="p-[20px] w-full bg-zinc-700 h-[200px] relative flex mb-[80px]">
                <div className="rounded-full w-[120px] h-[120px] bg-zinc-900 absolute bottom-[-60px] left-[20px]" />
            </header>
            <div className="p-[20px] flex flex-col">
                <h1 className="text-zinc-100 font-normal text-[30px]">Bem-vindo(a) à página do grupo {params.group}</h1>
                <div className="flex flex-col gap-4 mt-5">
                    <div className="w-full min-h-[30px] bg-zinc-700 p-[10px] rounded-lg">
                        post
                    </div>
                    <div className="w-full min-h-[30px] bg-zinc-700 p-[10px] rounded-lg">
                        post
                    </div>
                    <div className="w-full min-h-[30px] bg-zinc-700 p-[10px] rounded-lg">
                        post
                    </div>
                </div>
            </div>
        </div>
    )
}