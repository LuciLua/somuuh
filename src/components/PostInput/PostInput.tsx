...
function PostInput() {
    ...
    return (
        <>
            {user.username !== "anonymous" && session &&
                <div className="w-full bg-zinc-900 relative h-fit flex flex-col py-3">
                    <div className="flex w-full h-fit relative">
                        <div className="w-[70px] h-[70px] min-w-[70px] min-h-[70px]  max-w-[70px] max-h-[70px] m-3 overflow-hidden rounded-full relative flex">
                            <Image className="object-cover" src={user.image} fill alt="you" />
                        </div>
                        <div className="flex flex-col w-full relative h-fit justify-center ">
                            <input className="w-full min-h-[100px] h-fit bg-zinc-900 border-none p-4 outline-none text-zinc-400 placeholder:text-zinc-500" type="text" placeholder={`Olá, ${user.username}! Digite aqui sua postagem...`} />
